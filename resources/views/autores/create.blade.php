<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Novo Autor</h2>
    ...
    <div class="py-6 max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Preencha os dados do autor para cadastrá-lo na biblioteca</p>
            </div>
            
            <form method="POST" action="{{ route('autores.store') }}" class="px-6 py-6">
                @csrf
                <div class="space-y-6">
                    <div>
                        <x-input-label for="nome" value="Nome Completo" />
                        <x-text-input id="nome" name="nome" type="text" class="mt-1 block w-full" :value="old('nome')" required autofocus placeholder="Digite o nome completo do autor" />
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="nacionalidade" name="nacionalidade" type="text" class="mt-1 block w-full" :value="old('nacionalidade')" required placeholder="Ex: Brasileira, Americana" />
                            <x-input-error class="mt-2" :messages="$errors->get('nacionalidade')" />
                        </div>

                        <div>
                            <x-input-label for="ano_nascimento" value="Ano de Nascimento" />
                            <x-text-input id="ano_nascimento" name="ano_nascimento" type="number" class="mt-1 block w-full" :value="old('ano_nascimento')" required placeholder="Ex: 1965" />
