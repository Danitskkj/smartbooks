<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Editar Livro</h2>
    ...
    <div class="py-6 max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Editar Informações do Livro</h3>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Atualize os dados do livro "{{ $livro->titulo }}"</p>
            </div>
            
            <form method="POST" action="{{ route('livros.update', $livro) }}" class="px-6 py-6">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div class="sm:col-span-2">
                            ...
                            <x-text-input id="titulo" name="titulo" type="text" class="mt-1 block w-full" :value="old('titulo', $livro->titulo)" required autofocus placeholder="Digite o título do livro" />
                            <x-input-error class="mt-2" :messages="$errors->get('titulo')" />
                        </div>

                        <div>
                            <x-input-label for="ano_publicacao" value="Ano de Publicação" />
                            <x-text-input id="ano_publicacao" name="ano_publicacao" type="number" class="mt-1 block w-full" :value="old('ano_publicacao', $livro->ano_publicacao)" required placeholder="Ex: 2023" />
                        ...
                        <div>
                            ...
                            <x-text-input id="quantidade_disponivel" name="quantidade_disponivel" type="number" class="mt-1 block w-full" :value="old('quantidade_disponivel', $livro->quantidade_disponivel)" required placeholder="Ex: 5" />
                            <x-input-error class="mt-2" :messages="$errors->get('quantidade_disponivel')" />
                        </div>

                        <div>
                            <x-input-label for="genero" value="Gênero" />
                            <x-text-input id="genero" name="genero" type="text" class="mt-1 block w-full" :value="old('genero', $livro->genero)" required placeholder="Ex: Romance, Ficção, Terror" />
